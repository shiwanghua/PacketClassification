...
int simple_match(ACL_rules* a, message* p, int *_cycle){
	...
	for(int i=0;i<a->size;i++){
		...
		if (p->source_port < a->list[i].source_port[0] || a->list[i].source_port[1] < p->source_port)continue;  //if source port not match, check next
		if (p->destination_port < a->list[i].destination_port[0] || a->list[i].destination_port[1] < p->destination_port)continue;  //if destination port not match, check next
...
int match_with_log(Cell* _c, message* p, int *_cycle)
{
	uint64_t time_1, time_2;
	time_1 = GetCPUCycle();
	unsigned int e_protocol, es_ip, ed_ip;
...
void analyse_log(ACL_rules* data)
{
	...
	for (int i = 0; i < data->size; i++) {
		...
		{
		case 0:
			c_id[IP_LAYER_1] = IP_EDN_CELL_1;//int checkNum[],
			c_id[IP_LAYER_2] = IP_EDN_CELL_2;
			break;
		case 1:
			c_id[IP_LAYER_1] = p->source_ip[3] >> IP_WIDTH_1;
...
void analyse_log2(ACL_rules* data,char outputFileName[])
{
	...
	FILE* fp = NULL;
	fp = fopen(outputFileName, "w");
	fprintf(fp,"IP\tsrc1\tsrc2\tsrc3\tsrc4\tdst1\tdst2\tdst3\tdst4\n");
	for(int i=0;i<257;i++){
		fprintf(fp, "%d\t%4d\t%4d\t%4d\t%4d\t%4d\t%4d\t%4d\t%4d\n", i, srcIP[0][i],srcIP[1][i],srcIP[2][i],srcIP[3][i],dstIP[0][i],dstIP[1][i],dstIP[2][i],dstIP[3][i]);
	}
	fclose(fp);
}
